...
# Mapping of servo functions to GPIO pins and indexes
servo_map = {
...
# Build lists from the map
servo_pins = [servo_map[name]["gpio"] for name in servo_map]
servo_names = list(servo_map.keys())  # To keep name order consistent

GPIO.setmode(GPIO.BCM)
GPIO.setwarnings(False)

# Initialize PWM channels for servos at 50Hz
pwms = []
for pin in servo_pins:
    GPIO.setup(pin, GPIO.OUT)
    pwm = GPIO.PWM(pin, 50)
    pwm.start(7.5)  # Neutral starting duty
    pwms.append(pwm)
...
last_angles = [90, 90, 170, 5]  # Initial positions
...
def move_ease_in_out(index, target_angle, duration=1.0, steps=50):
    ...
    for i in range(steps + 1):
        t = i / steps
        eased_t = 0.5 * (1 - math.cos(math.pi * t))  # Ease-in-out
...
def move_servo(index, angle):
    ...
    if method == "Instant":
        ...
        # Read duration and steps from user inputs
        try:
...
# Threaded wrapper for non-blocking GUI
...
def slider_click(event, slider, index):
	# Get the click position and convert to a new value
	widget = event.widget
